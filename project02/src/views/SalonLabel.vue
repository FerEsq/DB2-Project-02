...
export default {
  ...
  methods: {
    ...
    return session
      .run('MATCH (u:Salon) RETURN u, ID(u) as id')
      .then(result => {
        this.users = result.records.map(record => {
          return {
            ...
            capacidad: record.get('u').properties.capacidad,
            disponible: record.get('u').properties.disponible,
            edificio: record.get('u').properties.edificio,
            nivel: record.get('u').properties.nivel
    ...
    deleteUser(userId) {
      ...
      session.writeTransaction((tx) => {
        ...
        tx.run('MATCH (u:Salon)-[r]-() WHERE ID(u) = $userId DELETE r', { userId: parseInt(userId) });
        
        // Luego eliminar el nodo
        return tx.run('MATCH (u:Salon) WHERE ID(u) = $userId DELETE u', { userId: parseInt(userId) });
      })
      .then(() => {
