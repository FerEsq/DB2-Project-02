...
export default {
  ...
  methods: {
    ...
    return session
      .run('MATCH (u:Club) RETURN u, ID(u) as id')
      .then(result => {
        this.users = result.records.map(record => {
          return {
            ...
            presupuesto: record.get('u').properties.presupuesto,
            dias: record.get('u').properties.dias,
            fechaFundacion: record.get('u').properties.fechaFundacion
    ...
    deleteUser(userId) {
      ...
      session.writeTransaction((tx) => {
        ...
        tx.run('MATCH (u:Club)-[r]-() WHERE ID(u) = $userId DELETE r', { userId: parseInt(userId) });
        
        // Luego eliminar el nodo
        return tx.run('MATCH (u:Club) WHERE ID(u) = $userId DELETE u', { userId: parseInt(userId) });
      })
      .then(() => {
