    ...
    <table class="user-table">
      ...
      <tbody>
        <tr v-for="user in users" :key="user.id">
          ...
          <td>{{ user.codigo }}</td>
          <td>{{ user.creditos }}</td>
          <td>{{ user.promedioAprobado }}</td>
          <td>{{ user.competencias }}</td>
          <td>
  ...
  methods: {
      ...
      .then(result => {
        this.users = result.records.map(record => {
          return {
            ...
            codigo: record.get('u').properties.codigo,
            creditos: record.get('u').properties.creditos,
            promedioAprobado: record.get('u').properties.promedioAprobado,
            competencias: record.get('u').properties.competencias
    ...
    deleteUser(userId) {
      ...
      session.writeTransaction((tx) => {
        ...
        tx.run('MATCH (u:Curso)-[r]-() WHERE ID(u) = $userId DELETE r', { userId: parseInt(userId) });
        
        // Luego eliminar el nodo
        return tx.run('MATCH (u:Curso) WHERE ID(u) = $userId DELETE u', { userId: parseInt(userId) });
      })
      .then(() => {
